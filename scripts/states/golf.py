...
esc_text_render = Decaying_Felt_Pen_Font.render('Press   ESC   to   leave   golf', True, (0, 0, 0))
...
executed = f'import scripts.states.levels.level_{golf_level}'
...
while True:
    ...
    # golf_ball_mask = golf_ball_surface_rect.to_surface(unsetcolor=(0, 0, 0, 0), setcolor=(255, 255, 255, 255))
    golf_ball_mask = pygame.mask.from_surface(golf_ball_surface_rect)
    ...
    vignette_alpha = 50 + overlap_sand_mask.count() // 10
    ...
    if start_fade == True:
        if fade_alpha <= 255:
            ...
            pygame.quit()
            os.system('python scripts\\states\\patient_room_2.py') # fixxx latr
            
    win.blit(esc_text_render, (210, 20))
